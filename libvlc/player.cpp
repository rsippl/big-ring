...
Mwindow::Mwindow(): speedingUp(true) {
    vlcPlayer = NULL;
...
void Mwindow::initComponents() {
    ...
    connect(timer, SIGNAL(timeout()), this, SLOT(changeRate()));
    timer->start(100);
...
void Mwindow::changeRate() {
	if (!vlcPlayer) 
		return;
	
	float currentRate = libvlc_media_player_get_rate(vlcPlayer);
	if (speedingUp) {
		libvlc_media_player_set_rate(vlcPlayer, currentRate * 1.005);
		speedingUp = (currentRate < 2.0);
	} else {
		libvlc_media_player_set_rate(vlcPlayer, currentRate * .995);
		speedingUp = (currentRate < .5);
	}
	speedLabel->setNum(currentRate);
}
